...
import { FiClock, FiCalendar, FiMessageSquare, FiFileText, FiActivity, FiDownload,FiStar } from 'react-icons/fi';
...
export default function PreviousMeetingPageDetails() {
  ...
  const generateSummary = async () => {
    const [duration,unit]=getMeetingDuration(callData.startAt,callData.endedAt).split(" ");
    const durationInNumber=Number(duration);
    if(unit==="sec"){
      toast.warning("Meeting duration is less than 1 min.Therefore, summary cannot be generated.");
      return;
    }
    else if(unit==="min" && durationInNumber<10){
      toast.warning(`Meeting duration is less than ${durationInNumber} min.Therefore, summary cannot be generated.`);
      return;
    }
    try {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <div className="flex items-center space-x-2 text-sm bg-white/5 px-4 py-2 rounded-lg">
                ...
                <span>{getMeetingDuration(callData.startAt,callData.endedAt)} </span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="glass-card p-6">
            <div className="flex flex-row items-center mb-4 gap-2 justify-between">
              ...
              <div className="flex items-center gap-2 " >
                ...
                {summary && (
                  <div className="p-2 bg-blue-500/20 rounded-lg cursor-pointer" title="Download Summary" onClick={()=>downloadSummaryById()}>
            ...
            <div className="bg-white/5 p-4 rounded-lg border border-white/10">
              {summary ? (
                ...
                <p className="text-gray-400 text-sm">
                  No AI summary available. The summary will be generated by click on the generate summary button.
                </p>
              )}
            </div>
            ...
            {/* Key Points */}
            <div className="mt-4">
              <h3 className="text-sm font-medium text-gray-300 mb-2">Key Points</h3>
                      ...
                      {msg.reaction_counts && Object.keys(msg.reaction_counts).length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {Object.entries(msg.reaction_counts).map(([reaction, count]) => (
                            ...
                            >
                              <span>{reaction}</span>
                              <span className="text-xs opacity-80">{String(count)}</span>
                            </span>
                          ))}
                        </div>
                      )}
       ...
       {!showChat && ( <div className="glass-card p-6">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="text-xl font-semibold text-white">Meeting Transcription</h2>
            <span className="text-xs bg-white/10 text-white px-2 py-1 rounded-full">
          ...
          {transcriptions.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-gray-300 font-medium">No transcription available</h3>
