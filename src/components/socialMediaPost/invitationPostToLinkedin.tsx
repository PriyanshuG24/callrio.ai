...
export const InvitationPostToLinkedin = ({
  meetingId,
}: InvitationPostToLinkedinProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [description, setDescription] = useState("");
  const meetingLink = `${window.location.origin}/dashboard/meeting/${meetingId}`;

  const handlePostToLinkedin = async () => {
    const { success, message } = await shareMeetingOnLinkedin({
      description,
      meetingLink,
    });
    setIsOpen(false);
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input value={meetingLink} disabled className="pl-9" />
              </div>
            </div>
          </div>
        </div>
